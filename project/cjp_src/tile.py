...
class TileURLTemplate(object):
    ...
    def __init__(self, template, format='png'):
        ...
        self.with_cjp_path = True if '%(cjp_path)' in template else False
    ...
    def substitute(self, tile_coord, format=None, grid=None):
        ...
        if self.with_cjp_path:
            data['cjp_path'] = cjp_path(tile_coord)
...
def cjp_path(tile_coord):
    """
    >>> tms_path((1234567, 87654321, 9))
    '9/1234567/87654321'
    """
    x = str(tile_coord[0]).zfill(7)
    y = str(tile_coord[1]).zfill(7)
    z = tile_coord[2]
    return '%s/%s%s/%s%s/%s%s/%s%s/%s%s/%s%s/%s%s' % (z,x[0],y[0],x[1],y[1],x[2],y[2],x[3],y[3],x[4],y[4],x[5],y[5],x,y)
